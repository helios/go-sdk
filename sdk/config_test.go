...
func TestConfigWithOptions(t *testing.T) {
	...
	config := getHeliosConfig(serviceName, token, WithCollectorInsecure(), WithCollectorEndpoint(testCollectorEndpoint), WithCollectorPath(testCollectorPath), WithSamplingRatio(testSamplingRatio), WithDebugMode())
	assert.Equal(t, config.apiToken, token)
